...
class Sighan14CSCTrainParser(HTMLParser):
  def __init__(self):
    super(Sighan14CSCTrainParser, self).__init__()
    self.data = []
  ...
  def handle_endtag(self, tag):
    ...
    if tag == 'essay':
      ...
      for j, (e, c) in enumerate(zip(input_text, target_text)):
      
        if (e != c):
          _correction.append([j + 1, c])
      self._data[mistake_id]['correction'] = _correction
...
class Sighan14CSCTestParser(object):
...
parser = Sighan14CSCTrainParser()


train_set = ['Training/B1_training.sgml', 'Training/C1_training.sgml']

test_set = [('Test/CLP14_CSC_TestInput.txt', 'Test/CLP14_CSC_TestTruth.txt')]

parser_test = Sighan14CSCTestParser('../dataset/NCU_NLPLab_CSC/clp14csc_release1.1')


for _train_set in train_set:
  with open(os.path.join('../dataset/NCU_NLPLab_CSC/clp14csc_release1.1', _train_set), 'r') as f:
    print(_train_set)
...
save_pickle(data, '../cache/sighancsc14.cache')
